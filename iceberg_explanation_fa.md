...
graph TD
    subgraph "مرحله ۱: استخراج و تبدیل (ETL)"
        ...
        A -- "۱. خواندن داده" --> B
        B -- "۲. پاکسازی و تبدیل داده (ETL)" --> C(DataFrame آماده برای ورود)
    end

    subgraph "مرحله ۲: نوشتن در آیسبرگ و تعامل با کاتالوگ"
        C -- "۳. نوشتن در جدول `logs`" --> D{جدول آیسبرگ: `logs`}
        D -- "۴. ایجاد فایل فراداده جدید" --> E(metadata-v2.json)
        F[کاتالوگ آیسبرگ] -- "۵. درخواست آپدیت" --> E
        F -- "۶. آپدیت اتمی اشاره‌گر (CAS)" --> G((Pointer: logs -> metadata-v2.json))
    end

    subgraph "مرحله ۳: پارتیشن‌بندی و بهینه‌سازی"
        D -- "۷. پارتیشن‌بندی پنهان" --> H{Partition Transform: `days(event_timestamp)`}
        H -- "مقدار `2023-10-23T14:00` به `2023-10-23` تبدیل می‌شود" --> I[فایل‌های Parquet در پارتیشن‌های روزانه]
    end

    subgraph "مرحله ۴: خواندن بهینه داده"
        J[کاربر/ابزار BI]
        J -- "۸. کوئری: `WHERE level = 'ERROR'`" --> K{موتور کوئری (Trino/Spark)}
        K -- "۹. درخواست فراداده از کاتالوگ" --> F
        K -- "۱۰. دریافت آدرس فراداده فعلی" --> G
        K -- "۱۱. اعمال Data Skipping" --> L{بررسی آمار (min/max) در مانیفست‌ها}
